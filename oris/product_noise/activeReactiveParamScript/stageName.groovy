//Below code is to configure stageName-activeChoiceReactiveParameter
//Referenced parameters:pipelineExecutionID,buildNumber
...
def  stageNames= data.hits.hits.collect { it._source.stage.name }.unique()

def parameters = []
stageNames.each { name ->
    parameters.add(name)
}
